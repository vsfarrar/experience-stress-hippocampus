...
cort_exp_log <-
  lmer(log(cort_conc) ~ as.factor(experience_chicks)*sex*as.factor(timepoint) + (1|bird_id), 
       data = horm_blood)
...
prl_exp_log <-
  lmer(log(prl_conc + 1) ~ as.factor(experience_chicks)*sex*as.factor(timepoint) + (1|bird_id), 
       data = horm_blood) 
...
emms1 <- emmeans(prl_exp_log, ~ stage*timepoint | sex)
...
emmeans(prl_exp_log, pairwise ~ as.factor(experience_chicks) | as.factor(timepoint)*sex) #shows that effect of experience is larger in females than in males across.
